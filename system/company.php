<?php
	switch($_GET['action'])
	{
		...
		break;
		case "delete":
			define_page("company_delete");
			$id = $_GET['id'];
			$db = mysql_query("DELETE from `company` where id='$id'");
			echo "<script>history.go(-1)</script>";	
		break;
		default:
			define_page("company_view");
			$num = $_GET['num'];
			$num == 0 ? $num=1 : $page=$num;
			$limit[0] = 0+30*($num-1);
			$limit[1] = 30*$num;
			$option =mysql_query("SELECT `name` from `company` where `customer`=1");
			if($_POST['Search'])
			{
				$name=$_POST['name'];
				$attn=$_POST['attn'];
				$db = mysql_query("SELECT * from `company` where `name` like '%$name%' and `attn` like '%$attn%' and `customer` =1  LIMIT $limit[0] , $limit[1]");

			}else{
				$db = mysql_query("SELECT * from `company`  where `customer`=1 LIMIT $limit[0] , $limit[1]");
			     }
...
include( dirname(dirname(__FILE__)).'/template/system/company.html');
